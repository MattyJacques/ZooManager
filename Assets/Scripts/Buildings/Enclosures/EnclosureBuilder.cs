...
namespace Assets.Scripts.Buildings.Enclosures
{
    ...
    {
        private enum State { Idle, GettingFirstCornerPosition, GettingSecondCornerPosition };
        ...
        private List<EnclosureComponent> _enclosures = new List<EnclosureComponent> ();
        ...
        private Vector3 _cornerRotationOffset = new Vector3(0f, 0f, 90f);
        //To account for the corner's pivot not being at 0 for the specific model used in the prototype
        private Vector3 _cornerPositionOffset = new Vector3(0f, -1f, 0f);
        private Vector3 _wallRotationOffset = new Vector3(0f, 0f, 90f);
        ...
        public EnclosureComponent GetEnclosureAtPos(Vector3 positionToCheck)
        { //Returns whatever enclosure that positionToCheck is inside of.
            //Returns null if there is no enclosure there.

            foreach (EnclosureComponent e in _enclosures)
            {
        ...
        { // Check to see if the parameters are valid
            ...
            return true;
        } // EnclosureParametersAreSane()

        private void DrawVisualizedEnclosure(Vector3 firstCorner, Vector3 secondCorner)
        { // Draw the enclosure so the user can preview what it will look like
        ...
        { // Place the encosure using the two provided Vector3s. Creates all of the wall gameobjects for
            // the enclosure
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (angle % 2 == 0)
                {
                    cornerObjects[i].transform.Rotate(_cornerRotationOffset * angle);
                }
                else
                {
                    int val = 1;
                ...
                cornerObjects[i].transform.Translate(_cornerPositionOffset, Space.Self);
                ...
                for (int x = 1; x < length - 1; x++)
                {
                    ...
                    g.transform.Rotate(_wallRotationOffset * (i + 1));
                }
            }

            return new[] { wallObjects, cornerObjects };
        } // BuildSquareEnclosure()

        private void FinalizeEnclosureBuild(Vector3 firstCorner, Vector3 secondCorner)
        { // Finalise the enclosure. Added the appropriate components, setting everything
            // up for use of enclosure by outside entities
            ...
            EnclosureComponent enclose = enclosure.GetComponent<EnclosureComponent>();
            if (enclose != null)
        ...
        { // Cancel the enclosure, cleaning everything up
            foreach (GameObject g in _tempCorners)
            {
                _pool.Destroy(EnclosureBuilderPool.ObjectType.Corner, g);
            }
