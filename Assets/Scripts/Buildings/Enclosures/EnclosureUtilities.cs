...
namespace Assets.Scripts.Buildings.Enclosures
{
    public class EnclosureUtilities : MonoBehaviour
    {
        ...
        static List<EnclosureComponent> _activeEnclosures = new List<EnclosureComponent>();

        static public void AddEnclosure(ref EnclosureComponent enclosure)
        ...
        static public bool IsActiveEnclosure(Vector3 position, ref EnclosureComponent enclosureRef)
        { // Get the reference to the enclosure if there is one. 

            bool result = false;

            foreach (EnclosureComponent enclosure in _activeEnclosures)
