...
public class Console : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_consoleEnabled && Input.GetMouseButtonDown(0) && !_consoleRect.Contains(new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y)))
        {
            RaycastHit hit;
            ...
            case spawn:
                ...
                if (inputParamsLength > 1)
                {
                    ...
                    for (int i = 0; i < amount; i++)
                    {
                        ...
                        {
                            objectToSpawn.transform.position = location.Substring(location.IndexOf('(')+1, location.Length - 2).Split(',').ParseVec3();
                        }
                        else
                        {
    ...
    //Private methods
    bool AddClassToGameObject(GameObject obj, string s)
    {
